/**
 * This file is part of the `prism-media` package.
 *
 * It has been modified here to allow for a custom FFmpeg
 * executable to be specified.
 */
...
const VERSION_REGEX = /version (.+) Copyright/i
...
class FFmpeg extends Duplex {
   ...
   * @param {boolean} [options.shell] Whether FFmpeg should be spawned inside a shell
   * @example
   ...
   * @param {boolean} [force] If true, will ignore any cached results and search for the command again
   * @returns {FFmpegInfo}
   ...
   * }
   */
  static getInfo(force = false, extraSource = '') {
    ...
    if (extraSource !== '') sources.unshift(extraSource)
   ...
   * @throws Will throw an error if FFmpeg cannot be found.
   */
  static create({ args = [], shell = false, source = '' } = {}) {
    if (!args.includes('-i'))
      args.unshift('-i', '-')
    return ChildProcess.spawn(FFmpeg.getInfo(false, source).command, args.concat(['pipe:1']), { windowsHide: true, shell })
  }
}
